	...
	datasets: [
		{
			...
			data: [<?php $first = false; for( $i=count( $dashboard_data ) - 1; $i >= 0; $i-- ){ if( $first ){ echo ","; }else{ $first = true; } echo ( $dashboard_data[$i]->total - $dashboard_data[$i]->discount_total - $dashboard_data[$i]->refund_total ); } ?>]
		}
	]
};
var options = {
